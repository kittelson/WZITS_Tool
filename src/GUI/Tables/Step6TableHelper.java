...
public class Step6TableHelper extends TableView {
    ...
    public static Node createStepSummary(MainController mc) {
        ...
        bPane.setTop(NodeFactory.createFormattedLabel("Fact Sheet #8: WZITS Operations and Maintenance", "fact-sheet-title-large"));
        ...
        stepLists.add(p.getQGen().qChangingConditionsList);
        stepLists.add(p.getQGen().qSharingInfoList);
        stepLists.add(p.getQGen().qStaffingList);
        stepLists.add(p.getQGen().qPublicSupportList);
        stepLists.add(p.getQGen().qMonitoringEvalList);
